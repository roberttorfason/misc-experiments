...
class LLWithVar(nn.Module):
    def forward(self, mean, beta, value):
        ll_tot = torch.log(beta) - beta * (value - mean)**2
        return -torch.mean(ll_tot)


def main():
    # t, y = input_data.generate_data(invert=False)
    t, y = input_data.generate_heteroscedastic_line()
    ...
    t_extended = np.arange(start=0.0, stop=5.0, step=0.01, dtype=np.float32)
    ...
    plt.figure()
    plt.plot(t_extended, 1.0 / net(torch.from_numpy(t_extended))[1].data.numpy(), 'r')
    plt.show()
